...
- **One continuously recorded Conflict-packet session** against the deployed,
  styled build at `LIVE_URL` in Chrome with `chrome://flags/#enable-webmcp-testing`
...
JSON result sits beside the form. The result lands and the **legal name** row
flips from “Not linked yet” to the bound value with its source. Persistent
on-screen label, top-left: **“Excerpt from 1:35 of this same recording.”**
...
Need-Based Scholarship / Fictional demo · Synthetic data only”. Click
**Start conflict packet**. The application page loads; point the cursor at the
line that reads **“WebMCP: six CiteApply tools registered.”**
...
supported fields. The form visibly updates as the result returns. *(This is the
...
decide.”** Then `get_validation_issues` listing the conflict and the undeclared
email as blockers.
...
**On screen:** in the visible portal, pick a reason from **Why this source**,
click **Use `<document>`: `<value>`** for the source you stand behind, then in
the email row click **Save email** and **I declare this is my address**.

> "So I decide. I pick the source I'm willing to stand behind, and I say why. And
> I declare my own contact address. Those two acts are mine — there is no tool
...
content hash, and the conflict warning carried through.

> "Same hash, same warning, on a receipt I could defend. Synthetic records, a
